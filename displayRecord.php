...
<section id="appointmentPart">
  <div class="container">
    <h3 class="text-center">HISTORIA CLÍNICA DEL PACIENTE</h3>
...
<form id="borang2" name="borang2" method="post" action="updateDetails2.php">
 ...
 <table width="380"  align="center" cellspacing="12px">
   ...
   <h3>REGISTRO DE TRATAMIENTO DENTAL</h3>
   ...
   <?php
   $result1 = mysqli_query($conn, "SELECT * FROM treatment_record WHERE id  = $id order by treatment_date DESC") or die("Error Running MySQL query");
  ...
  {
    $result2 = mysqli_query($conn, "SELECT * FROM treatment_record WHERE  id  = $id order by treatment_date DESC") or die("Error Running MySQL query");
    while ($row1 = mysqli_fetch_assoc($result2))
    {

    ?>
   <h5>FECHA DE TRATAMIENTO: <?php echo $row1["treatment_date"]; ?></h5><br>
   <h5>TIPO DE TRATAMIENTO: <?php echo $row1["treatment_type"]; ?></h5><br>
   <h5>NOTA DE TRATAMIENTO: <?php echo $row1["treatment_note"] ?></h5>
        <div>
